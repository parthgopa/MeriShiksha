  ...
  const categories = [
    {
      ...
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-[var(--accent-teal)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      return (
        <div className="animate-fadeIn mt-4" ref={careerCraftingRef}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {getSubcategories("Career Crafting").map((subcategory, subIndex) => {
              ...
              return subcategoryPages.map((page, index) => (
                <div 
                  ...
                  className={`transition-all duration-500 transform ${animateItems ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
                  style={{ transitionDelay: `${(subIndex * 0.1) + (index * 0.05)}s` }}
        ...
        <div className="space-y-8">
          {categories.map((category) => (
            <div key={category.id} className="relative">
              {/* Category Header */}
              <div 
                className={`relative cursor-pointer transition-all duration-300 transform hover:translate-x-1 ${selectedCategory === category.id ? 'translate-x-2' : ''}`}
                onClick={() => handleCategoryClick(category.id)}
              >
                <div className={`flex items-center p-4 sm:p-5 rounded-t-lg ${selectedCategory === category.id ? 'bg-gradient-to-r from-[var(--primary-black)] to-[var(--primary-violet)]/30 border-t border-l border-r border-[var(--accent-teal)]/30' : 'hover:bg-[var(--primary-violet)]/10'}`}>
                  <div className={`mr-3 sm:mr-4 ${selectedCategory === category.id ? 'text-[var(--accent-teal)]' : 'text-gray-400'}`}>
                    {category.icon}
                  </div>
                  <div className="flex-1">
                    <h3 className={`text-lg sm:text-xl font-bold transition-all duration-300 ${selectedCategory === category.id ? 'text-[var(--accent-teal)]' : 'text-white'}`}>
                      {category.title}
                    </h3>
                  </div>
                  <div className="ml-2">
                    <svg 
                      ...
                      className={`h-5 w-5 transition-transform duration-300 ${selectedCategory === category.id ? 'rotate-180 text-[var(--accent-teal)]' : 'text-gray-400'}`} 
                      fill="none" 
                      viewBox="0 0 24 24" 
                      stroke="currentColor"
                    >
              ...
              {/* Category Content */}
              {selectedCategory === category.id && (
                <div className="bg-gradient-to-br from-[var(--primary-black)] to-[var(--primary-violet)]/20 border border-[var(--accent-teal)]/30 rounded-b-lg rounded-tr-lg p-4 sm:p-6 shadow-lg">
